...
#**************** ACCOUNTS API ****************
  @getAccountList
  @ignore
  Scenario: GetAccountList should succeed
    * def req =
    """
    ...
    And match xml contains '<fjs1:account>'
    And string accountData = get xml /*[name()='SOAP-ENV:Envelope']/*[name()='SOAP-ENV:Body']/*[name()='fjs1:getAccountListResponseFlow']/*[name()='fjs1:getAccountListResponse']/*[name()='fjs1:account'][1]
    And xml accountData = accountData.replaceAll("fjs1:", "")

  Scenario Outline: GetAccountActivity should succeed
    * def accountListScenario = call read("@{this}@getAccountList") {customerNumber: "<customerNumber>"}
    * xml accountData = accountListScenario.accountData
    * def req =
    """
    <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:amp="http://soprabanking.com/amplitude">
        <soapenv:Header/>
            ...
            <amp:getAccountActivityRequestFlow>
                ...
                <amp:getAccountActivityRequest>
                    <amp:accountIdentifier>
                        <amp:branch>#(accountData.account.branch.code)</amp:branch>
                        <amp:currency>#(accountData.account.account.accountNumber.internalFormatAccountOurBranch.currency.alphaCode)</amp:currency>
                        <amp:account>#(accountData.account.account.accountNumber.internalFormatAccountOurBranch.account)</amp:account>
                    </amp:accountIdentifier>
                    <amp:movementPeriod>
                        <amp:startDate>2020-01-01Z</amp:startDate>
                    </amp:movementPeriod>
                </amp:getAccountActivityRequest>
            </amp:getAccountActivityRequestFlow>
        </soapenv:Body>
    ...
    Then status 200
    Then xmlstring xml = response
    Then match xml contains '<fjs1:getAccountActivityResponse>'
    Examples:
    | validCustomers |
